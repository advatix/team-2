    ...
    if (this.ssotoken) {
      this.loginService.getWmsToken(this.ssotoken).subscribe(
        data => {
          ...
          var shippers = (this.responce.responseObject.shipper) ? this.responce.responseObject.shipper : [];
          if (shippers.length > 0) {
            if (this.responce.responseObject.role == 1) {
              this.router.navigate(['/orders/viewAllOrders']);
            } else {
              this.router.navigate(['/welcome']);
            }
          } else {
            this.router.navigate(['/nolocation']);
          }
        },
        err => {
